...
export default function SellerProfile() {
  ...
  const { data, isLoading, error } = useQuery('sellerData', fetchUserData, {
  ...
  useEffect(() => {
    if (data?.profile) {
      setProfileData({
        name: data.profile.sellerFields?.name || '',
        email: data.profile.email || '',
        businessName: data.profile.sellerFields?.businessName || '',
        contactNumber: data.profile.sellerFields?.contactNumber || '',
      })
      ...
      <section className="seller-profile-section py-20 bg-gray-100">
        <div className="container mx-auto px-4">
          ...
          <div className="bg-white rounded-lg shadow-lg p-8">
            ...
            {updateProfileMutation.isError && <div className="text-center text-red-600 mb-4">{updateProfileMutation.error?.message || 'An error occurred'}</div>}
